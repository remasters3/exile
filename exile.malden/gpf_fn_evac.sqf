...
[_EvacHeliV,_HeliPadT,_TargetPosMarker] spawn {_EvacHeliV =_this select 0;_HeliPadT =_this select 1;_TargetPosMarker =_this select 2;
        while {alive _HeliPadT} Do {
		  if ((_HeliPadT distance _EvacHeliV) < 10) Then {deleteMarkerLocal _TargetPosMarker;sleep 10;DeleteVehicle _HeliPadT;};
		  sleep 1;
		  };
...
_way3Pos = [_TargetPos,3000,4000, 5, 1, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
...
[_vehicles,_markers] Spawn {
	   ...
	   {deleteVehicle _x} foreach units _EvacHeliGroup;
