...
_Timeout = 600;
_patrol = false;
_models = [(SelectRandom _Model),(SelectRandom _Model),(SelectRandom _Model),(SelectRandom _Model),(SelectRandom _Model),(SelectRandom _Model)];
...
if (_patrol) then {[_DropTroopsGroup, _DropPos, 50,[]] call BIS_fnc_taskPatrol;} else {
 [_vic,_DropTroopsGroup] Spawn { _vic = _this select 0; _DropTroopsGroup = _this select 1;
  while {alive _vic} Do {_DropPos = GetPos _vic; _way = _DropTroopsGroup addWaypoint [_DropPos, 0];_way setWaypointType "Move";_way setWaypointSpeed "FULL";Sleep 10;};
 }; 
};
{removeBackpack _x;_x addBackpack "B_Parachute";_x moveInCargo _DropHeliV;} Foreach units _DropTroopsGroup;
