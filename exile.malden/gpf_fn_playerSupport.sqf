...
[_player] Spawn {_player = _this select 0;
  ...
  _GetInVehicle = {_unit = _this select 0; _veh = _this select 1;
	...
	if ((_veh emptyPositions "Gunner") > 0) Then {[_unit] orderGetIn true;_unit assignAsGunner _veh;};/*_unit moveInGunner _veh;*/
	if ((_veh emptyPositions "Commander") > 0) Then {[_unit] orderGetIn true;_unit assignAsCommander _veh;};/*_unit moveInCommander _veh;*/
	if ((_veh emptyPositions "Cargo") > 0) Then {[_unit] orderGetIn true;_unit assignAsCargo _veh;};/*_unit moveInCargo _veh;*/
	};
  };
